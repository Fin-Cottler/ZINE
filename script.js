...
import { getFirestore, collection, addDoc, setDoc, getDoc, getDocs, doc, updateDoc, deleteDoc, query, where, orderBy, getCountFromServer} from "https://www.gstatic.com/firebasejs/10.0.0/firebase-firestore.js";
...
export const newProject = async function(){
  try {
      ...
      var collectionName = "§"+ username + key;
      var access = new Date();
      ...
      //displays all projects
        showProjects();
        sessionStorage.setItem('projectName',collectionName);
...
export const showProjects = async function() {
  ...
  // Convert snapshot to an array of { id, data }
  const projects = snapshot.docs.map(doc => ({
    id: doc.id,
    data: doc.data()
  }));

  // Sort projects by accessDate (newest first)
  // Ensure accessDate is a Firestore Timestamp or Date
  projects.sort((a, b) => {
    const aDate = a.data.accessDate?.toDate ? a.data.accessDate.toDate() : new Date(a.data.accessDate);
    const bDate = b.data.accessDate?.toDate ? b.data.accessDate.toDate() : new Date(b.data.accessDate);
    return bDate - aDate; // descending
  });

  // Render projects in sorted order
  let count = 0;
  projects.forEach(({ id, data }) => {
    // Create project container
    const project = document.createElement("div");
    project.className = `project ${count % 2 === 0 ? "gray" : "white"}`;

    // Click box
    const click = document.createElement("div");
    click.className = "click-box";
    ...
    logo.className = "material-symbols-outlined";
    logo.textContent = "edit_document";
    ...
    label.innerHTML = "Access Date:";


    // Access date
    const access = document.createElement("div");
    access.className = "access-date";
    const date = data.accessDate?.toDate ? data.accessDate.toDate() : new Date(data.accessDate);
    access.textContent = date.toLocaleString();
    ...
    button.onclick = () => deleteProject(data.projectName);

    // Assemble
    click.appendChild(logo);
    click.appendChild(name);
    click.appendChild(label);
    click.appendChild(access);
    project.appendChild(click);
    project.appendChild(button);
    allProjectsDiv.appendChild(project);
...
export const deleteProject = async function(projName){
...
export const loadProject = async function () {
  ...
  var q = query(collection(db, "collection-names"), where("projectName", "==", project));

  var querySnapshot = await getDocs(q);

  if (querySnapshot.empty) {
    console.log(`No documents found for project "${project}".`);
    return;
  }

  var access = new Date();
  for (const item of querySnapshot.docs) {
    const updateItem = doc(db, "collection-names", item.id);
    try {
      await updateDoc(updateItem, { 
        accessDate: access
      });
      console.log(`✅ Updated ${item.id} → displayName: ${newName}`);
    } catch (err) {
...
export const saveProject =  async function(){
  ...
  for (var i = 0; i < allPages.length; i++) {
    if (!(allPageNums.includes(allPages[i].id))){
      await addDoc(collection(db, project), {
          pageNumber: allPages[i].id,  
          format: allPages[i].className
      });
    }
  }

  var newName = document.getElementById("project-title").textContent.trim();

  // Build a query to only fetch docs where projName == project
  var q = query(collection(db, "collection-names"), where("projectName", "==", project));
  ...
  allDocs.forEach((item) => {
      ...
      if(item.data().pageNumber != null) {
        ...
        //update display name within collection
        //update display name within collection-names
      }
  })
...
export const loadProjectPrint = async function () {
  ...
  for (let i = 1; i < pages.length/2; i++) { 
    if (i%2 == 1) {
      pages[i*2].id = i;
      pages[(i*2)+1].id = (pages.length - 2) - (i - 1);;
    }else if (i%2 == 0) {
      pages[i*2].id = (pages.length - 2) - (i - 1);;
      pages[(i*2)+1].id = i;
    }
  }
